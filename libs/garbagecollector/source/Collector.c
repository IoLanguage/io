...
float Collector_marksPerAlloc(Collector *self)
{
	return self->marksPerAlloc;
}
...
float Collector_allocatedStep(Collector *self)
{
	return self->allocatedStep;
}
...
size_t Collector_freeAllValues(Collector *self)
{
	size_t count = 0;
	CollectorFreeFunc *freeFunc = self->freeFunc;
	COLLECTOR_FOREACH(self, v, (*freeFunc)(v); CollectorMarker_free(v); count ++;);
	self->allocated -= count;

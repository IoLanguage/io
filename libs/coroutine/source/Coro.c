...
/*
void Coro_startCoro_(Coro *self, Coro *other, void *context, CoroStartCallback
*callback)
{
...
void Coro_StartWithArg(unsigned int hiArg, unsigned int loArg) {
    CallbackBlock *block =
        (CallbackBlock *)(((long long)hiArg << 32) | (long long)loArg);
...
void Coro_setup(Coro *self, void *arg) {
    ...
    self->fiber =
        CreateFiber(Coro_stackSize(self),
                    (LPFIBER_START_ROUTINE)Coro_StartWithArg, (LPVOID)arg);
    if (!self->fiber) {
...
void Coro_setup(Coro *self, void *arg) {
    ...
    */
    ...
    self->env[2] = 0;
    self->env[3] =
        (unsigned long)(Coro_stack(self)) + Coro_stackSize(self) - 64;
...
#define SUN_STACK_END_INDEX 4
                                        ...
                                        // SunOS 9
                                        buf[SUN_PROGRAM_COUNTER] =
(JBTYPE)Coro_Start;

                                        x = (JBTYPE)stack;
                                        while ((x % 8) != 0) x --; // align on
an even boundary buf[SUN_STACK_START_INDEX] = (JBTYPE)x; x =
(JBTYPE)((JBTYPE)stack-stacksize / 2 + 15); while ((x % 8) != 0) x ++; // align
on an even boundary buf[SUN_STACK_END_INDEX] = (JBTYPE)x;

...
size_t CTYPE_size(CTYPE type) {
    ...
    case CTYPE_float32_t:
        return sizeof(float32_t);
    case CTYPE_float64_t:
...
int CTYPE_forName(const char *name) {
    if (!strcmp(name, "uint8"))
        return CTYPE_uint8_t;
    if (!strcmp(name, "uint16"))
        return CTYPE_uint16_t;
    if (!strcmp(name, "uint32"))
        return CTYPE_uint32_t;
    if (!strcmp(name, "uint64"))
        return CTYPE_uint64_t;
    if (!strcmp(name, "int8"))
        return CTYPE_int8_t;
    if (!strcmp(name, "int16"))
        return CTYPE_int16_t;
    if (!strcmp(name, "int32"))
        return CTYPE_int32_t;
    if (!strcmp(name, "int64"))
        return CTYPE_int64_t;
    if (!strcmp(name, "float32"))
        return CTYPE_float32_t;
    if (!strcmp(name, "float64"))
        return CTYPE_float64_t;
    return -1;
}

int CTYPE_fixedWidthTextEncodingForType(CTYPE type) {
    if (type == CTYPE_int8_t)
        return CENCODING_ASCII;
    if (type == CTYPE_uint16_t)
        return CENCODING_UCS2;
    if (type == CTYPE_uint32_t)
        return CENCODING_UCS4;
    return -1;
}
...
int CENCODING_forName(const char *name) {
    if (!strcmp(name, "ascii"))
        return CENCODING_ASCII;
    if (!strcmp(name, "utf8"))
        return CENCODING_UTF8;
    if (!strcmp(name, "ucs2"))
        return CENCODING_UCS2;
    if (!strcmp(name, "ucs4"))
        return CENCODING_UCS4;
    if (!strcmp(name, "number"))
        return CENCODING_NUMBER;
    return -1;
}
...
void UArray_unsupported_with_(const UArray *self, const char *methodName,
                              const UArray *other) {
    // UArray_error_(self, "Error: '%s' not supported between '%s' and '%s'\n");
    printf("Error: '%s' not supported between '%s' and '%s'\n", methodName,
           CTYPE_name(self->itemType), CTYPE_name(other->itemType));
    exit(-1);
}
...
void UArray_setItemType_(UArray *self, CTYPE type) {
    ...
    if (UArray_isFloatType(self)) {
        ...
        case 2:
            self->encoding = CENCODING_UCS2;
            break;
        case 4:
            self->encoding = CENCODING_UCS4;
            break;
        case 8:
            self->encoding = CENCODING_NUMBER;
            break;
...
UArray *UArray_newWithData_type_encoding_size_copy_(void *bytes, CTYPE type,
                                                    CENCODING encoding,
...
void UArray_print(const UArray *self) {
    if (self->encoding == CENCODING_ASCII || self->encoding == CENCODING_UTF8) {
        ...
        printf("[");
        UARRAY_FOREACH(self, i, v, printf("%f", (float)v);
                       if (i != self->size - 1) printf(", "););
        printf("]");
    } else {
        printf("[");
        UARRAY_FOREACH(self, i, v, printf("%i", (int)v);
                       if (i != self->size - 1) printf(", "););
        printf("]");
    }
}

UArray UArray_stackAllocedWithData_type_size_(void *data, CTYPE type,
                                              size_t size) {
    UArray self;
...
UArray *UArray_newWithCString_copy_(char *s, int copy) {
    return UArray_newWithData_type_size_copy_(s, CTYPE_uint8_t, strlen(s),
                                              copy);
}

UArray *UArray_newWithCString_(const char *s) {
    return UArray_newWithData_type_size_copy_((uint8_t *)s, CTYPE_uint8_t,
                                              strlen(s), 1);
}
...
void UArray_checkIfOkToRelloc(UArray *self) {
    if (self->stackAllocated) {
        printf("UArray debug error: attempt to io_realloc UArray data that "
               "this UArray does not own");
        exit(-1);
    }
}
#endif

void UArray_setData_type_size_copy_(UArray *self, void *data, CTYPE type,
                                    size_t size, int copy) {
    size_t sizeInBytes;
...
const void *UArray_data(const UArray *self) { return self->data; }

const uint8_t *UArray_bytes(const UArray *self) { return self->data; }
...
void UArray_setSize_(UArray *self, size_t size) {
    if (size != self->size) {
        ...
        if (newSizeInBytes > oldSizeInBytes) {
            memset(self->data + oldSizeInBytes, 0,
...
size_t UArray_size(const UArray *self) { return self->size; }

size_t UArray_sizeInBytes(const UArray *self) {
    return self->size * self->itemSize;
}

void UArray_sizeTo_(UArray *self, size_t size) { UArray_setSize_(self, size); }
...
void UArray_removeRange(UArray *self, size_t start, size_t removeSize) {
    if (start < self->size) {
        if (start + removeSize > self->size) {
            ...
            memmove(UARRAY_BYTESAT_(self, start),
                    UARRAY_BYTESAT_(self, remainder),
                    self->itemSize * remainderSize);
        }
...
void UArray_leave_thenRemove_(UArray *self, size_t itemsToLeave,
...

    {
...

        {
            ...
            size_t newItemCount =
                chunkCount * itemsToLeave + tailChunkSizeInBytes / itemSize;
...

            {
                ...
                for (chunkPos = 0; chunkPos < chunkCount; chunkPos++) {
                    memmove(newData + chunkPos * chunkSizeInBytes,
                            UARRAY_BYTESAT_(self, chunkPos * period),
                            chunkSizeInBytes);
                }
                ...
                if (tailChunkSizeInBytes) {
                    memmove(newData + chunkPos * chunkSizeInBytes,
                            UARRAY_BYTESAT_(self, chunkPos * period),
                            tailChunkSizeInBytes);
                }

                UArray_setData_type_size_copy_(
                    self, newData, UArray_itemType(self), newItemCount, 0);
                UArray_changed(self);
            }
        }
    }
...
#define UARRAY_RAWAT_PUT_(self, pos, v)                                        \
    ...
    case CTYPE_uint8_t:                                                        \
        ((uint8_t *)self->data)[pos] = v;                                      \
        return;                                                                \
    case CTYPE_uint16_t:                                                       \
        ((uint16_t *)self->data)[pos] = v;                                     \
        return;                                                                \
    case CTYPE_uint32_t:                                                       \
        ((uint32_t *)self->data)[pos] = v;                                     \
        return;                                                                \
    case CTYPE_uint64_t:                                                       \
        ((uint64_t *)self->data)[pos] = v;                                     \
        return;                                                                \
    case CTYPE_int8_t:                                                         \
        ((int8_t *)self->data)[pos] = v;                                       \
        return;                                                                \
    case CTYPE_int16_t:                                                        \
        ((int16_t *)self->data)[pos] = v;                                      \
        return;                                                                \
    case CTYPE_int32_t:                                                        \
        ((int32_t *)self->data)[pos] = v;                                      \
        return;                                                                \
    case CTYPE_int64_t:                                                        \
        ((int64_t *)self->data)[pos] = v;                                      \
        return;                                                                \
    case CTYPE_float32_t:                                                      \
        ((float32_t *)self->data)[pos] = v;                                    \
        return;                                                                \
    case CTYPE_float64_t:                                                      \
        ((float64_t *)self->data)[pos] = v;                                    \
        return;                                                                \
    case CTYPE_uintptr_t:                                                      \
        ((uintptr_t *)self->data)[pos] = v;                                    \
        return;                                                                \
    }
...
void UArray_at_putPointer_(UArray *self, size_t pos, void *v) {
    ...
    // UArray_error_(self, "UArray_at_putPointer_ not supported with this
    // type");
}
...
void UArray_appendBytes_size_(UArray *self, const uint8_t *bytes, size_t size) {
    UArray a = UArray_stackAllocedWithData_type_size_((uint8_t *)bytes,
                                                      CTYPE_uint8_t, size);
...
{
        UArray a = UArray_stackAllocedWithData_type_size_(&byte, CTYPE_uint8_t,
1); UArray_append_(self, &a);
}
*/
...

    {
        ...
        for (i = 0; i < selfSizeInBytes; i += chunkSize) {
            if (i + chunkSize > selfSizeInBytes) {
                UArray_appendBytes_size_(out, self->data + i,
                                         selfSizeInBytes - i);
            } else {
...
void UArray_at_putAll_(UArray *self, size_t pos, const UArray *other) {
...

    {
...

        {
            ...
            UArray newChunk =
                UArray_stackRange(self, pos + other->size, chunkSize);
            ...
            if (newChunk.size) // UArray_copy_(&newChunk, &oldChunk); // copy
                               // chunk to end
...
#define UARRAY_COMPARE_TYPES(OP2, TYPE1, self, TYPE2, other)                   \
    {                                                                          \
        size_t i,                                                              \
            minSize = self->size < other->size ? self->size : other->size;     \
        for (i = 0; i < minSize; i++) {                                        \
...
#define UARRAY_FIND_TYPES(OP2, TYPE1, self, TYPE2, other)                      \
    {                                                                          \
        size_t i;                                                              \
        if ((self->size < other->size) || (self->size == 0))                   \
            return -1;                                                         \
    ...
    {                                                                          \
        ...
        for (i = 0; i < self->size - other->size + 1; i++) {                   \
            ...
            for (j = 0; j < other->size; j++) {                                \
                TYPE1 v1 = ((TYPE1 *)self->data)[i + j];                       \
                TYPE2 v2 = ((TYPE2 *)other->data)[j];                          \
                if (tolower((int)v1) != tolower((int)v2)) {                    \
                    match = 0;                                                 \
                    break;                                                     \
                }                                                              \
            }                                                                  \
...
int cmp_uint8_t(const uint8_t *a, const uint8_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_uint16_t(const uint16_t *a, const uint16_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_uint32_t(const uint32_t *a, const uint32_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_uint64_t(const uint64_t *a, const uint64_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}

int cmp_int8_t(const int8_t *a, const int8_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_int16_t(const int16_t *a, const int16_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_int32_t(const int32_t *a, const int32_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_int64_t(const int64_t *a, const int64_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}

int cmp_float32_t(const float32_t *a, const float32_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_float64_t(const float64_t *a, const float64_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
int cmp_uintptr_t(const uintptr_t *a, const uintptr_t *b) {
    return *a == *b ? 0 : (*a < *b ? -1 : 1);
}
...
void UArray_sort(UArray *self) {
    ...
    switch (self->itemType) {
    case CTYPE_uint8_t:
        qsort(base, size, sizeof(uint8_t), (UArraySortCallback *)cmp_uint8_t);
        return;
    case CTYPE_uint16_t:
        qsort(base, size, sizeof(uint16_t), (UArraySortCallback *)cmp_uint16_t);
        return;
    case CTYPE_uint32_t:
        qsort(base, size, sizeof(uint32_t), (UArraySortCallback *)cmp_uint32_t);
        return;
    case CTYPE_uint64_t:
        qsort(base, size, sizeof(uint64_t), (UArraySortCallback *)cmp_uint64_t);
        return;

    case CTYPE_int8_t:
        qsort(base, size, sizeof(int8_t), (UArraySortCallback *)cmp_int8_t);
        return;
    case CTYPE_int16_t:
        qsort(base, size, sizeof(int16_t), (UArraySortCallback *)cmp_int16_t);
        return;
    case CTYPE_int32_t:
        qsort(base, size, sizeof(int32_t), (UArraySortCallback *)cmp_int32_t);
        return;
    case CTYPE_int64_t:
        qsort(base, size, sizeof(int64_t), (UArraySortCallback *)cmp_int64_t);
        return;

    case CTYPE_float32_t:
        qsort(base, size, sizeof(float32_t),
              (UArraySortCallback *)cmp_float32_t);
        return;
    case CTYPE_float64_t:
        qsort(base, size, sizeof(float64_t),
              (UArraySortCallback *)cmp_float64_t);
        return;
    case CTYPE_uintptr_t:
        qsort(base, size, sizeof(uintptr_t),
              (UArraySortCallback *)cmp_uintptr_t);
        return;
    }
}
...
void UArray_sortBy_(UArray *self, UArraySortCallback *cmp) {
    ...
    switch (self->itemType) {
    case CTYPE_uint8_t:
        qsort(base, size, sizeof(uint8_t), cmp);
        return;
    case CTYPE_uint16_t:
        qsort(base, size, sizeof(uint16_t), cmp);
        return;
    case CTYPE_uint32_t:
        qsort(base, size, sizeof(uint32_t), cmp);
        return;
    case CTYPE_uint64_t:
        qsort(base, size, sizeof(uint64_t), cmp);
        return;

    case CTYPE_int8_t:
        qsort(base, size, sizeof(int8_t), cmp);
        return;
    case CTYPE_int16_t:
        qsort(base, size, sizeof(int16_t), cmp);
        return;
    case CTYPE_int32_t:
        qsort(base, size, sizeof(int32_t), cmp);
        return;
    case CTYPE_int64_t:
        qsort(base, size, sizeof(int64_t), cmp);
        return;

    case CTYPE_float32_t:
        qsort(base, size, sizeof(float32_t), cmp);
        return;
    case CTYPE_float64_t:
        qsort(base, size, sizeof(float64_t), cmp);
        return;
    case CTYPE_uintptr_t:
        qsort(base, size, sizeof(uintptr_t), cmp);
        return;
    }
}

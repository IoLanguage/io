...
// metadoc PointerHash notes Suggestion to use cuckoo hash and original
// implementation by Marc Fauconneau
...
void PointerHash_setSize_(PointerHash *self, size_t size) {
    ...
    if (size > self->size) {
        memset(self->records + self->size * sizeof(PointerHashRecord), 0x0,
               (size - self->size) * sizeof(PointerHashRecord));
    }
...
void PointerHash_insertRecords(PointerHash *self, unsigned char *oldRecords,
                               size_t oldSize) {
    size_t i;

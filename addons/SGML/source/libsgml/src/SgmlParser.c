...
#define STRDUP(v) strcpy(malloc(strlen(v)+1), v)
...
{
	SGML_PARSER *ret = (SGML_PARSER *)calloc(1, sizeof(SGML_PARSER));
...
void _sgmlParserAppendBuffer(SGML_PARSER *parser, const char *chunk, unsigned long startOffset, unsigned long endOffset)
{
	...
	if (!parser->internal.currentBuffer)
		parser->internal.currentBuffer = (char *)calloc(1, copyLength + 1);
	else
...
/* SLD - this always seems to release non-malloced memory
				if (parser->internal.lastElementName)
					free(parser->internal.lastElementName);*/
...
/* SLD - this seems to always release non-malloced memory
				if (parser->internal.lastElementName)
					free(parser->internal.lastElementName);
					*/

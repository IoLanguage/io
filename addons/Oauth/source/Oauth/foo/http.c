...
{
	const size_t capacity = resp->bodySize - resp->bodyLen;

	if (capacity < n)
	{
		resp->bodySize += n - capacity;
		resp->body 	= (char *)realloc((void *)resp->body, sizeof(char) * resp->bodySize);
	}
}
...
int HttpRequest(struct url_props *url, const char *method, const struct string *headers, const struct string *content, struct http_response *resp)
{
	...
	if (headers)
	{
		EnsureBodyCapacity(resp, string_len(headers));
	...
	{
		EnsureBodyCapacity(resp, string_len(content) + 1024);
...
readAgain:
			...
			else
			{
				...
				if (chunkedEncoding)
				{
					...
					EnsureBodyCapacity(resp, resp->headersSize + need + 32);
